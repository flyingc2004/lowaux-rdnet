...
class ReflectionSythesis_1(object):
    ...
    def __init__(
        ...
        self.ghost_max_shift = ghost_max_shift
        self.ghost_strength = ghost_strength
        print('[i] reflection sythesis model: {}'.format({
            ...
            'low_alpha': low_alpha, 'high_alpha': high_alpha,
            'ghost_probability': ghost_probability, 'ghost_max_shift': ghost_max_shift,
    ...
    def __call__(self, B, R):
        ...
        if kernel_size <= 0 or kernel_size % 2 == 0:
            raise ValueError('kernel size must be a positive odd integer, got {}'.format(kernel_size))
        sigma = np.random.uniform(self.low_sigma, self.high_sigma)
        gamma = np.random.uniform(self.low_gamma, self.high_gamma)
        alpha = np.random.uniform(self.low_alpha, self.high_alpha)
        R_blur = np.empty_like(R_)
        kernel = cv2.getGaussianKernel(kernel_size, sigma)
        kernel2d = np.dot(kernel, kernel.T)

        for i in range(3):
            R_blur[...,i] = convolve2d(R_[...,i], kernel2d, mode='same')

        R_blur *= alpha
        if self.ghost_max_shift > 0 and np.random.random() < self.ghost_probability:
            dx = np.random.randint(-self.ghost_max_shift, self.ghost_max_shift + 1)
            dy = np.random.randint(-self.ghost_max_shift, self.ghost_max_shift + 1)
            if dx != 0 or dy != 0:
                shift = np.float32([[1, 0, dx], [0, 1, dy]])
                shifted = cv2.warpAffine(
                    R_blur,
                    shift,
                    (R_blur.shape[1], R_blur.shape[0]),
                    borderMode=cv2.BORDER_REFLECT,
                )
                R_blur = (1 - self.ghost_strength) * R_blur + self.ghost_strength * shifted
