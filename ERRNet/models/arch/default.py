...
class DRNet(torch.nn.Module):
    ...
    resblock_dilations=None):
        ...
        # Residual layers
        dilation_config = resblock_dilations or [1] * n_resblocks
        if len(dilation_config) != n_resblocks:
            raise ValueError('expected {} residual block dilations, got {}'.format(
                n_resblocks, dilation_config))
...
class DilatedRefiner(nn.Module):
    def __init__(self, in_channels=9, out_channels=3, channels=32, dilations=None):
        super().__init__()
        dilations = dilations or [1, 2, 4, 2, 1]
        act = nn.ReLU(True)
        self.head = ConvLayer(nn.Conv2d, in_channels, channels, kernel_size=3, stride=1, norm=None, act=act)
        self.body = nn.Sequential(*[
            ResidualBlock(channels, dilation=dilation, norm=None, act=act, res_scale=1)
            for dilation in dilations
        ])
        self.tail = ConvLayer(nn.Conv2d, channels, out_channels, kernel_size=3, stride=1, norm=None, act=None)

    def zero_init_output(self):
        nn.init.zeros_(self.tail.conv2d.weight)
        if self.tail.conv2d.bias is not None:
            nn.init.zeros_(self.tail.conv2d.bias)
