...
class SSIMLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        mu_predict = F.avg_pool2d(predict, self.window_size, stride=1, padding=padding)
        mu_target = F.avg_pool2d(target, self.window_size, stride=1, padding=padding)

        mu_predict_sq = mu_predict.pow(2)
        mu_target_sq = mu_target.pow(2)
        mu_product = mu_predict * mu_target

        sigma_predict = F.avg_pool2d(predict * predict, self.window_size, stride=1, padding=padding) - mu_predict_sq
        sigma_target = F.avg_pool2d(target * target, self.window_size, stride=1, padding=padding) - mu_target_sq
        sigma_product = F.avg_pool2d(predict * target, self.window_size, stride=1, padding=padding) - mu_product

        c1 = 0.01 ** 2
        c2 = 0.03 ** 2
        ssim = ((2 * mu_product + c1) * (2 * sigma_product + c2))
        ssim /= (mu_predict_sq + mu_target_sq + c1) * (sigma_predict + sigma_target + c2)
        return 1 - ssim.mean()
...
def init_loss(opt, tensor):
    ...
    if opt.pixel_loss == 'legacy':
        pixel_loss.initialize(MultipleLoss([nn.MSELoss(), GradientLoss()], [0.2, 0.4]))
    elif opt.pixel_loss == 'charbonnier_gradient':
        pixel_loss.initialize(MultipleLoss([CharbonnierLoss(), GradientLoss()], [1.0, opt.lambda_gradient]))
    else:
        raise ValueError("pixel loss [%s] not recognized." % opt.pixel_loss)
