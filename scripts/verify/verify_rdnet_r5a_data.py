...
def verify_config(config_path: Path, xreflection_root: Path):
    ...
    if int(train.get("size", -1)) != 7932:
        raise AssertionError("R5a train dataset size is not 7932")

    fused = train["fused_datasets"]
    types = [dataset["type"] for dataset in fused]
    ratios = [float(dataset["ratio"]) for dataset in fused]
    if types != EXPECTED_TYPES:
        raise AssertionError(f"Unexpected R5a dataset types: {types}")
    if not np.allclose(ratios, EXPECTED_RATIOS, atol=1e-12, rtol=0):
        raise AssertionError(f"Unexpected R5a dataset ratios: {ratios}")
    if abs(sum(ratios) - 1.0) > 1e-12:
        raise AssertionError("R5a dataset ratios do not sum to 1")
    if any(dataset["type"] == "SharpReflectionDataset" for dataset in fused):
        raise AssertionError("R5a must not include SharpReflectionDataset")
    ...
    for dataset in fused:
        for key in ("datadir", "manifest"):
            ...
            if any(token.lower() in value for token in forbidden):
                raise AssertionError(f"Forbidden R5a training source: {value}")
    ...
    if cfg["train"]["reflection_target"] != expected_reflection:
        raise AssertionError(
            f"Unexpected R5a reflection config: {cfg['train']['reflection_target']}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "/mnt/a/ljz/DIP/ERRNet/results/rdnet_configs/"
            "rdnet_r5a_rrw_only_from_r4a_e1_dry.yml"
        ),
    )
