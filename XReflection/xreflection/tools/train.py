...
def main():
    ...
    trainer_kwargs = {
        ...
        'num_sanity_val_steps': config['lightning'].get('num_sanity_val_steps', 2),
        'limit_train_batches': config['lightning'].get('limit_train_batches', 1.0),
        'limit_val_batches': config['lightning'].get('limit_val_batches', 1.0),
        'strategy': "deepspeed_stage_1",
    }
    
    # Add strategy for distributed training if specified
    if config['lightning'].get('strategy'):
        strategy_name = config['lightning']['strategy']
        ddp_timeout_seconds = config['lightning'].get('ddp_timeout_seconds')
        strategy_kwargs = {}
        if ddp_timeout_seconds is not None:
            strategy_kwargs['timeout'] = timedelta(seconds=int(ddp_timeout_seconds))

        if strategy_name == 'ddp_static_graph':
            strategy = DDPStrategy(static_graph=True, **strategy_kwargs)
        elif strategy_name == 'ddp_find_unused_parameters_true':
            strategy = DDPStrategy(find_unused_parameters=True, **strategy_kwargs)
        elif strategy_name == 'ddp' and strategy_kwargs:
            strategy = DDPStrategy(**strategy_kwargs)
        else:
